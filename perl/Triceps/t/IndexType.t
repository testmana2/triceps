...
# Comparisons with the hashed index
{
	###################### equality #################################
...
# Comparisons with the ordered index
{
	###################### equality #################################
	...
	ok(ref $it1, "Triceps::IndexType");
	$res = $it1->getIndexId();
	ok($res, &Triceps::IT_ORDERED);

	$it1n = Triceps::IndexType->newOrdered(key => [ "!a", "!b" ]);
	ok(ref $it1n, "Triceps::IndexType");
	...
	$it3 = Triceps::IndexType->newOrdered(key => [ "c", "d" ]);
	ok(ref $it3, "Triceps::IndexType");
	$it4 = Triceps::IndexType->newOrdered(key => [ "e" ]);
	ok(ref $it4, "Triceps::IndexType");
	$it5 = Triceps::IndexType->newFifo();
	ok(ref $it5, "Triceps::IndexType");
	$res = $it5->getIndexId();
	...
	# ordered index checks the match with the rowtype translation
	# if initialized
	{
		my $xrt1 = Triceps::RowType->new(
			a => int32,
	...
	ok($res, "index OrderedIndex(a, b, ) {\n  aggregator (\n    row {\n      uint8 a,\n      int32 b,\n      int64 c,\n      float64 d,\n      string e,\n    }\n  ) agg\n}");
	$res = $it1->print(undef);
	ok($res, "index OrderedIndex(a, b, ) { aggregator ( row { uint8 a, int32 b, int64 c, float64 d, string e, } ) agg }");
}
